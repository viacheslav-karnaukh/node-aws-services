...
const getResourceEffect = (authorizationToken: string): 'Allow' | 'Deny' => {
  ...
  return storedUserPassword && storedUserPassword === password ? 'Allow' : 'Deny';
};

const generatePolicy = (authorizationToken: string, resourceArn: string): APIGatewayAuthorizerResult => {
...
const basicAuthorizer: APIGatewayAuthorizerHandler = async (event: APIGatewayTokenAuthorizerEvent, _context) => {
  if(event.type !== 'TOKEN') {
